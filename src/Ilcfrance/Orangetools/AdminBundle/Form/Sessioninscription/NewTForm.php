...
class NewTForm extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options)
	{
		...
		if (null != $this->sessionformation) {
			...
			$builder->add('sessionformation', EntityidType::class, array(
				...
				'query_builder' => function (SessionformationRepository $sfr) use ($sessionformation_id) {
					return $sfr->createQueryBuilder('sf')->where('sf.id = :id')->setParameter('id', $sessionformation_id);
				},
				'choice_label' => 'id',
				'multiple' => false,
				'by_reference' => true,
			...
			$builder->add('sessionformation', EntityType::class, array(
				'label' => 'ilcfrance.orangetools.admin.SessioninscriptionNewForm.sessionformation.label',
				'class' => 'IlcfranceOrangetoolsDataBundle:Sessionformation',
				'query_builder' => function (SessionformationRepository $sfr) {
					$now = new \DateTime('now');
					return $sfr->createQueryBuilder('sf')->leftJoin('sf.sessioninscriptions', 'si')->where('sf.lockout = :sfLockout')->andWhere('sf.dtStart > :now')->groupBy('si.id')->addGroupBy('sf.id')->having('COUNT(si) < sf.maxParticipants')->setParameter('sfLockout', Sessionformation::LOCKOUT_UNLOCKED)->setParameter('now', $now->format('Y-m-d H:i:s'))->orderBy('sf.code', 'ASC');
				},
		...
		if (null != $this->user) {
			...
			$builder->add('user', EntityidType::class, array(
				...
				'query_builder' => function (UserRepository $ur) use ($user_id) {
					return $ur->createQueryBuilder('u')->where('u.id = :id')->setParameter('id', $user_id);
				},
				'choice_label' => 'id',
				'multiple' => false,
				'by_reference' => true,
			...
			$builder->add('user', EntityType::class, array(
				...
				'query_builder' => function (UserRepository $ur) {
					return $ur->createQueryBuilder('u')->join('u.userRoles', 'r')->where('u.lockout = :uLockout')->andWhere('r.name = :role')->setParameter('uLockout', User::LOCKOUT_UNLOCKED)->setParameter('role', 'ROLE_TRAINEE')->orderBy('u.username', 'ASC');
				},
				'choice_label' => 'choiceLabel',
				'multiple' => false,
				'by_reference' => true,
		...
		$builder->add('convocation', ChoiceType::class, array(
