...
class GroupmoduleController extends SasedevController
{
	...
	public function exportAction()
	{
		...
		$phpExcelObject->getProperties()->setCreator("Salah Abdelkader Seif Eddine")->setLastModifiedBy($this->getSecurityTokenStorage()->getToken()->getUser()->getFullname())->setTitle($this->translate('ilcfrance.orangetools.admin.Groupmodule.list.browserpagetitle'))->setSubject($this->translate('ilcfrance.orangetools.admin.Groupmodule.list.browserpagetitle'))->setDescription($this->translate('ilcfrance.orangetools.admin.Groupmodule.list.browserpagetitle'))->setKeywords($this->translate('ilcfrance.orangetools.admin.Groupmodule.list.browserpagetitle'))->setCategory("Groupmodule");
		...
		$filename = $this->normalize($this->getParameter('sitename') . '_' . $this->translate('ilcfrance.orangetools.admin.Groupmodule.list.browserpagetitle') . '_' . $now->format('Y-m-d_H.i.s'));
	...
	public function addPostAction()
	{
		...
		if (isset($reqData['GroupmoduleNewForm'])) {
			...
			if ($groupmoduleNewForm->isValid()) {
				...
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Groupmodule.add.success', array(
					'%groupmodule%' => $groupmodule->getName()
				)));

				return $this->redirect($this->generateUrl('ilcfrance_orangetools_admin_groupmodule_editGet', array(
					'id' => $groupmodule->getId()
				)));
			} else {

				$this->addFlash('error', $this->translate('ilcfrance.orangetools.admin.Groupmodule.add.failure'));
			}
		}
	...
	public function deleteAction($id)
	{
		...
		try {
			...
			if (null == $groupmodule) {
				...
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Groupmodule.delete.success', array(
					'%groupmodule%' => $groupmodule->getName()
				)));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($id)
	{
		...
		try {
			...
			if (null == $groupmodule) {
				...
				$moduleformationNewForm = $this->createForm(ModuleformationNewTForm::class, $moduleformation, array(
				...
				$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Groupmodule.edit.browserpagetitle', array(
					'%groupmodule%' => $groupmodule->getName()
				)));
				$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Groupmodule.edit.pagetitle', array(
					'%groupmodule%' => $groupmodule->getName()
				)));

				$dm = $this->container->get('doctrine_mongodb')->getManager();
				$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllByEntityId($groupmodule->getId(), Trace::AE_Groupmodule);
			...
			if (null == $groupmodule) {
				...
				if (isset($reqData['GroupmoduleUpdateNameForm'])) {
					...
					if ($groupmoduleUpdateNameForm->isValid()) {
						$em->persist($groupmodule);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Groupmodule.edit.success', array(
							'%groupmodule%' => $groupmodule->getName()
						)));
						...
						$this->addFlash('error', $this->translate('ilcfrance.orangetools.admin.Groupmodule.edit.failure'));
					}
					...
					if ($moduleformationNewForm->isValid()) {
						...
						$this->addFlash('error', $this->translate('ilcfrance.orangetools.admin.Moduleformation.add.error'));
					}
				}
