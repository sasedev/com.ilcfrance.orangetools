...
class TraineeController extends SasedevController
{
	...
	public function sendInfosAction(Request $request)
	{
		...
		foreach ($trainees as $trainee) {
			...
			if ($trainee->getInfoSent() == User::INFOSENT_NO) {
				...
				try {
					...
					$subject = '[' . $this->getParameter('sitename') . '] ' . $this->translate('ilcfrance.orangetools.admin.Trainee.mail.params.subject', array(
						'%year%' => date('Y')
					));
					$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($trainee->getEmail(), $trainee->getFullname())->setReplyTo($replyTo, $replyToName)->setSubject($subject)->setBody($this->renderView('IlcfranceOrangetoolsAdminBundle:Trainee:sendInfos.mail.html.twig', $mvars), 'text/html');
		...
		$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Trainee.mail.params.sent', array(
	...
	public function exportAction(Request $request)
	{
		...
		$phpExcelObject->getProperties()->setCreator("Salah Abdelkader Seif Eddine")->setLastModifiedBy($this->getSecurityTokenStorage()->getToken()->getUser()->getFullname())->setTitle($this->translate('ilcfrance.orangetools.admin.Trainee.list.browserpagetitle'))->setSubject($this->translate('ilcfrance.orangetools.admin.Trainee.list.browserpagetitle'))->setDescription($this->translate('ilcfrance.orangetools.admin.Trainee.list.browserpagetitle'))->setKeywords($this->translate('ilcfrance.orangetools.admin.Trainee.list.browserpagetitle'))->setCategory("Trainee");
		...
		$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->getFont()->setBold(true);
		$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->applyFromArray($headerStyle);
		$col++;
		...
		foreach ($trainees as $trainee) {
			...
			$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->getFont()->setBold(true);
			$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->applyFromArray($row % 2 == 1 ? $style1 : $style2);
			$col++;
			...
			foreach ($trainee->getModulepreinscriptions() as $modulepreinscription) {
				...
				$workSheet->setCellValueByColumnAndRow($col, $row, $modulepreinscription->getModuleformation()->getCode());
				$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->getFont()->setBold(true);
				$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->applyFromArray($row % 2 == 1 ? $style2 : $style1);
				$col++;
			}

			foreach ($trainee->getSessioninscriptions() as $sessioninscription) {

				$workSheet->setCellValueByColumnAndRow($col, $row, $sessioninscription->getSessionformation()->getCode());
				$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->getFont()->setBold(true);
				$workSheet->getCellByColumnAndRow($col, $row)->getStyle()->applyFromArray($row % 2 == 1 ? $style1 : $style2);
				$col++;
			}
		...
		$filename = $this->normalize($this->getParameter('sitename') . '_' . $this->translate('ilcfrance.orangetools.admin.Trainee.list.browserpagetitle') . '_' . $now->format('Y-m-d_H.i.s'));
	...
	public function addPostAction(Request $request)
	{
		...
		if (isset($reqData['TraineeNewForm'])) {
			...
			if ($traineeNewForm->isValid()) {
				...
				$em->persist($trainee);
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Trainee.add.success', array(
					'%user%' => $trainee->getUsername()
				)));

				return $this->redirect($this->generateUrl('ilcfrance_orangetools_admin_trainee_editGet', array(
					'id' => $trainee->getId()
				)));
			} else {
		...
		if (isset($reqData['TraineeImportForm'])) {
			...
			if ($traineeImportForm->isValid()) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if ($xlsUsername != "") {
						$trainee = $em->getRepository('IlcfranceOrangetoolsDataBundle:User')->findOneBy(array(
								...
								if ($cellValue != '') {
									...
									foreach ($sessionformations as $sessionformation) {
										if ($cellValue != '' && $sessionformation->getCode() == $cellValue) {
											...
											$sessioninscription = $em->getRepository('IlcfranceOrangetoolsDataBundle:Sessioninscription')->findOneBy(array(
												'user' => $trainee,
	...
	public function deleteAction($id, Request $request)
	{
		...
		try {
			...
			if (null == $user) {
				...
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Trainee.delete.success', array(
	...
	public function editGetAction($id, Request $request)
	{
		...
		try {
			...
			if (null == $trainee) {
				...
				$modulepreinscription = new Modulepreinscription();
				$modulepreinscription->setUser($trainee);
				$modulepreinscriptionNewForm = $this->createForm(ModulepreinscriptionNewTForm::class, $modulepreinscription, array(
					'user' => $trainee
				));
				...
				$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Trainee.edit.browserpagetitle', array(
					'%user%' => $trainee->getUsername()
				)));
				$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Trainee.edit.pagetitle', array(
					'%user%' => $trainee->getUsername()
				)));

				$dm = $this->container->get('doctrine_mongodb')->getManager();
				$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllByEntityId($trainee->getId(), Trace::AE_User);
			...
			if (null == $trainee) {
				...
				if (isset($reqData['TraineeUpdateInfosForm'])) {
					...
					if ($userUpdateInfosForm->isValid()) {
						$em->persist($trainee);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Trainee.edit.success', array(
							'%user%' => $trainee->getUsername()
						)));
					...
					if ($userSendParamsForm->isValid()) {
						...
						try {
							...
							$message = \Swift_Message::newInstance()->setFrom($from, $fromName)->setTo($trainee->getEmail(), $trainee->getFullname())->setSubject($subject)->setBody($this->renderView('IlcfranceOrangetoolsAdminBundle:Trainee:sendInfos.mail.html.twig', $mvars), 'text/html');
					...
					if ($modulepreinscriptionNewForm->isValid()) {
						$em->persist($modulepreinscription);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Modulepreinscription.add.success', array(
