...
class AdminController extends SasedevController
{
	...
	public function listAction(Request $request)
	{
		...
		$roleAdmin = $em->getRepository('IlcfranceOrangetoolsDataBundle:Role')->findOneBy(array(
			'name' => 'ROLE_ADMIN'
		));
		if (null == $roleAdmin) {
	...
	public function addPostAction(Request $request)
	{
		...
		if (isset($reqData['AdminNewForm'])) {
			...
			if ($adminNewForm->isValid()) {
				...
				$em->persist($admin);
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Admin.add.success', array(
					'%user%' => $admin->getUsername()
				)));

				return $this->redirect($this->generateUrl('ilcfrance_orangetools_admin_admin_editGet', array(
					'id' => $admin->getId()
				)));
			} else {

				$this->addFlash('error', $this->translate('ilcfrance.orangetools.admin.Admin.add.failure'));
			}
		}
	...
	public function deleteAction($id, Request $request)
	{
		...
		try {
			...
			if (null == $user) {
				...
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Admin.delete.success', array(
	...
	public function editGetAction($id, Request $request)
	{
		...
		try {
			...
			if (null == $admin) {
				...
				$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Admin.edit.browserpagetitle', array(
					'%user%' => $admin->getUsername()
				)));
				$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Admin.edit.pagetitle', array(
					'%user%' => $admin->getUsername()
				)));

				$dm = $this->container->get('doctrine_mongodb')->getManager();
				$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllByEntityId($admin->getId(), Trace::AE_User);
			...
			if (null == $admin) {
				...
				if (isset($reqData['AdminUpdateEmailForm'])) {
					...
					if ($userUpdateEmailForm->isValid()) {
						$em->persist($admin);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Admin.edit.success', array(
							'%user%' => $admin->getUsername()
						)));
						...
						$this->addFlash('error', $this->translate('ilcfrance.orangetools.admin.Admin.edit.failure'));
					}
				}
