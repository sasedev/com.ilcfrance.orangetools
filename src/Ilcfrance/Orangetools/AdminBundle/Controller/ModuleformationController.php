...
class ModuleformationController extends SasedevController
{
	...
	public function listByGroupmoduleAction($id)
	{
		...
		$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Moduleformation.listByGroupmodule.browserpagetitle', array(
			'%groupmodule%' => $groupmodule->getName()
		)));
		$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Moduleformation.listByGroupmodule.pagetitle', array(
			'%groupmodule%' => $groupmodule->getName()
		)));
	...
	public function exportAction()
	{
		...
		$phpExcelObject->getProperties()->setCreator("Salah Abdelkader Seif Eddine")->setLastModifiedBy($this->getSecurityTokenStorage()->getToken()->getUser()->getFullname())->setTitle($this->translate('ilcfrance.orangetools.admin.Moduleformation.list.browserpagetitle'))->setSubject($this->translate('ilcfrance.orangetools.admin.Moduleformation.list.browserpagetitle'))->setDescription($this->translate('ilcfrance.orangetools.admin.Moduleformation.list.browserpagetitle'))->setKeywords($this->translate('ilcfrance.orangetools.admin.Moduleformation.list.browserpagetitle'))->setCategory("Moduleformation");
		...
		foreach ($moduleformations as $moduleformation) {
			$i++;
			$workSheet->setCellValue('A' . $i, $moduleformation->getGroupmodule()->getName(), \PHPExcel_Cell_DataType::TYPE_STRING2);
		...
		$filename = $this->normalize($this->getParameter('sitename') . '_' . $this->translate('ilcfrance.orangetools.admin.Moduleformation.list.browserpagetitle') . '_' . $now->format('Y-m-d_H.i.s'));
	...
	public function addPostAction()
	{
		...
		if (isset($reqData['ModuleformationNewForm'])) {
			...
			if ($moduleformationNewForm->isValid()) {
				...
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Moduleformation.add.success', array(
					'%moduleformation%' => $moduleformation->getCode()
				)));

				return $this->redirect($this->generateUrl('ilcfrance_orangetools_admin_moduleformation_editGet', array(
			...
			if ($moduleformationImportForm->isValid()) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if ($xlsGroupmodule != "" && $xlsCode != "" && $xlsTitle != "") {
						$groupmodule = $em->getRepository('IlcfranceOrangetoolsDataBundle:Groupmodule')->findOneBy(array(
							'name' => $xlsGroupmodule
						));
						if (null != $groupmodule) {
	...
	public function deleteAction($id)
	{
		...
		try {
			...
			if (null == $moduleformation) {
				...
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Moduleformation.delete.success', array(
					'%moduleformation%' => $moduleformation->getCode()
				)));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($id)
	{
		...
		try {
			...
			if (null == $moduleformation) {
				...
				$sessionformationNewForm = $this->createForm(SessionformationNewTForm::class, $sessionformation, array(
				...
				$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Moduleformation.edit.browserpagetitle', array(
					'%moduleformation%' => $moduleformation->getCode()
				)));
				$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Moduleformation.edit.pagetitle', array(
					'%moduleformation%' => $moduleformation->getCode()
				)));

				$dm = $this->container->get('doctrine_mongodb')->getManager();
				$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllByEntityId($moduleformation->getId(), Trace::AE_Moduleformation);
			...
			if (null == $moduleformation) {
				...
				if (isset($reqData['ModuleformationUpdateGroupmoduleForm'])) {
					...
					if ($moduleformationUpdateGroupmoduleForm->isValid()) {
						$em->persist($moduleformation);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Moduleformation.edit.success', array(
							'%moduleformation%' => $moduleformation->getCode()
						)));
