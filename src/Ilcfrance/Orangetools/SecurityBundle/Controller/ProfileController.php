...
class ProfileController extends SasedevController
{
	...
	public function profileGetAction()
	{
		...
		$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllRelatedToUser($user);
	...
	public function profilePostAction()
	{
		...
		if (isset($reqData['UserUpdateEmailForm'])) {
			...
			if ($userUpdateEmailForm->isValid()) {
				$em->persist($user);
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.security.UserUpdateEmailForm.success'));

				return $this->redirect($urlFrom);
			} else {
				$em->refresh($user);

				$this->addFlash('error', $this->translate('ilcfrance.orangetools.security.UserUpdateEmailForm.error', array(
			...
			if ($userUpdatePasswordForm->isValid()) {
				...
				$this->addFlash('error', $this->translate('ilcfrance.orangetools.security.UserUpdatePasswordForm.error'));
			}
			...
			if ($userUpdateInfosForm->isValid()) {
				...
				$this->addFlash('error', $this->translate('ilcfrance.orangetools.security.UserUpdateInfosForm.error'));
			}
			...
			if ($userUpdateMobileForm->isValid()) {
				...
				$this->addFlash('error', $this->translate('ilcfrance.orangetools.security.UserUpdateMobileForm.error'));
			}
		}
