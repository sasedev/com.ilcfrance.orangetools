...
class SessionformationController extends SasedevController
{
	...
	public function listByModuleformationAction($id)
	{
		...
		$sessionformations = $em->getRepository('IlcfranceOrangetoolsDataBundle:Sessionformation')->getAllByModuleformation($moduleformation);
		$this->addTwigVar('sessionformations', $sessionformations);
		$this->addTwigVar('moduleformation', $moduleformation);

		$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Sessionformation.listByModuleformation.browserpagetitle', array(
			'%moduleformation%' => $moduleformation->getCode()
		)));
		$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Sessionformation.listByModuleformation.pagetitle', array(
			'%moduleformation%' => $moduleformation->getCode()
		)));
	...
	public function exportAction()
	{
		...
		$phpExcelObject->getProperties()->setCreator("Salah Abdelkader Seif Eddine")->setLastModifiedBy($this->getSecurityTokenStorage()->getToken()->getUser()->getFullname())->setTitle($this->translate('ilcfrance.orangetools.admin.Sessionformation.list.browserpagetitle'))->setSubject($this->translate('ilcfrance.orangetools.admin.Sessionformation.list.browserpagetitle'))->setDescription($this->translate('ilcfrance.orangetools.admin.Sessionformation.list.browserpagetitle'))->setKeywords($this->translate('ilcfrance.orangetools.admin.Sessionformation.list.browserpagetitle'))->setCategory("Sessionformation");
		...
		foreach ($sessionformations as $sessionformation) {
			$i++;
			$workSheet->setCellValue('A' . $i, $sessionformation->getModuleformation()->getCode(), \PHPExcel_Cell_DataType::TYPE_STRING2);
			...
			$workSheet->getStyle('D' . $i)->getNumberFormat()->setFormatCode('yyyy-mm-dd h:mm:ss');
			...
			$workSheet->getStyle('E' . $i)->getNumberFormat()->setFormatCode('yyyy-mm-dd h:mm:ss');
			...
			$workSheet->setCellValue('J' . $i, \html_entity_decode(\strip_tags($sessionformation->getOtherInfos())), \PHPExcel_Cell_DataType::TYPE_STRING2);
			...
			$workSheet->setCellValue('M' . $i, $this->translate('Sessionformation.lockout.' . $sessionformation->getLockout()), \PHPExcel_Cell_DataType::TYPE_STRING2);
		...
		$filename = $this->normalize($this->getParameter('sitename') . '_' . $this->translate('ilcfrance.orangetools.admin.Sessionformation.list.browserpagetitle') . '_' . $now->format('Y-m-d_H.i.s'));
	...
	public function addPostAction()
	{
		...
		if (isset($reqData['SessionformationNewForm'])) {
			...
			if ($sessionformationNewForm->isValid()) {
				...
				$em->flush();
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Sessionformation.add.success', array(
					'%sessionformation%' => $sessionformation->getCode()
				)));

				return $this->redirect($this->generateUrl('ilcfrance_orangetools_admin_sessionformation_editGet', array(
			...
			if ($sessionformationImportForm->isValid()) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if ($xlsModuleformation != "" && $xlsCode != "" && $xlsTitle != "" && $xlsDtStart != null && $xlsDtEnd != null && $xlsLocation != "" && $xlsMaxParticipants != "") {
						$moduleformation = $em->getRepository('IlcfranceOrangetoolsDataBundle:Moduleformation')->findOneBy(array(
							'code' => $xlsModuleformation
						));
						if (null != $moduleformation) {
							$sessionformation = $em->getRepository('IlcfranceOrangetoolsDataBundle:Sessionformation')->findOneBy(array(
								'code' => $xlsCode
							));
	...
	public function deleteAction($id)
	{
		...
		try {
			...
			if (null == $sessionformation) {
				...
				$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Sessionformation.delete.success', array(
					'%sessionformation%' => $sessionformation->getCode()
				)));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($id)
	{
		...
		try {
			...
			if (null == $sessionformation) {
				...
				$sessionformationUpdateModuleformationForm = $this->createForm(SessionformationUpdateModuleformationTForm::class, $sessionformation);
				...
				$sessionformationUpdatePhoneContactCenterForm = $this->createForm(SessionformationUpdatePhoneContactCenterTForm::class, $sessionformation);
				$sessionformationUpdateConditionsReportForm = $this->createForm(SessionformationUpdateConditionsReportTForm::class, $sessionformation);
				...
				$sessionformationUpdateMaxParticipantsForm = $this->createForm(SessionformationUpdateMaxParticipantsTForm::class, $sessionformation);
				...
				$this->addTwigVar('SessionformationUpdatePhoneContactCenterForm', $sessionformationUpdatePhoneContactCenterForm->createView());
				...
				$this->setBrowserPageTitle($this->translate('ilcfrance.orangetools.admin.Sessionformation.edit.browserpagetitle', array(
					'%sessionformation%' => $sessionformation->getCode()
				)));
				$this->setPageTitle($this->translate('ilcfrance.orangetools.admin.Sessionformation.edit.pagetitle', array(
					'%sessionformation%' => $sessionformation->getCode()
				)));

				$dm = $this->container->get('doctrine_mongodb')->getManager();
				$traces = $dm->getRepository('IlcfranceOrangetoolsDataBundleMongodb:Trace')->getAllByEntityId($sessionformation->getId(), Trace::AE_Sessionformation);
			...
			if (null == $sessionformation) {
				...
				if (isset($reqData['SessionformationUpdateModuleformationForm'])) {
					...
					if ($sessionformationUpdateModuleformationForm->isValid()) {
						$em->persist($sessionformation);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Sessionformation.edit.success', array(
							'%sessionformation%' => $sessionformation->getCode()
						)));
					...
					if ($sessioninscriptionSendConvocationForm->isValid()) {
						...
						foreach ($sessionformation->getSessionInscriptions() as $sessioninscription) {
							...
							if ($sessioninscription->getConvocation() == Sessioninscription::CONVOCATION_NOTSENT) {
								...
								try {
									...
									$mvars['plan'] = $message->embed(\Swift_Image::fromPath($this->getParameter('kernel.root_dir') . '/../web/images/plan.jpg'));
									...
									$message->setFrom($from, $fromName)->setTo($trainee->getEmail(), $trainee->getFullname());
									if (null != $trainee->getEmail2()) {
										$message->setBcc($trainee->getEmail2(), $trainee->getFullName2());
									}
									$message->setReplyTo($replyTo, $replyToName)->setSubject($subject)->setBody($this->renderView('IlcfranceOrangetoolsAdminBundle:Sessioninscription:sendConvocation.mail.html.twig', $mvars), 'text/html');

									$this->sendmail($message);
									$sessioninscription->setConvocation(Sessioninscription::CONVOCATION_SENT);
									$em->persist($sessioninscription);

									$modulepreinscription = $em->getRepository('IlcfranceOrangetoolsDataBundle:Modulepreinscription')->findOneBy(array(
										'moduleformation' => $sessioninscription->getSessionformation()->getModuleformation(),
						...
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Sessioninscription.mail.convocation.sent', array(
							'%countSessioninscriptions%' => $countSessioninscriptions,
							'%$mailsent%' => $mailsent
						)));
					...
					if ($sessioninscriptionNewForm->isValid()) {
						$em->persist($sessioninscription);
						$em->flush();
						$this->addFlash('success', $this->translate('ilcfrance.orangetools.admin.Sessioninscription.add.success', array(
